...
# ── FIREBASE AUTH ────────────────────────────────────────────────
def get_web_cfg():
    try: return dict(st.secrets["firebase_web"])
    except: return {}
...
def login_page():
    cfg=get_web_cfg()
    if not cfg:
        st.error("Firebase belum dikonfigurasi. Isi .streamlit/secrets.toml"); st.stop()
    cfg_json=json.dumps(cfg)
    components.html(f"""
<script src="https://www.gstatic.com/firebasejs/9.22.2/firebase-app-compat.js"></script>
<script src="https://www.gstatic.com/firebasejs/9.22.2/firebase-auth-compat.js"></script>
<div style="font-family:sans-serif;text-align:center;padding:2.5rem 1rem">
  <div style="font-size:3rem;margin-bottom:.6rem">&#127891;</div>
  <div style="font-size:1.4rem;font-weight:700;margin-bottom:.3rem">CAT MHI</div>
  <div style="font-size:.85rem;opacity:.55;margin-bottom:2rem;line-height:1.5">Mediator Hubungan Industrial<br>Ahli Madya</div>
  <button id="gb" onclick="signIn()" style="display:inline-flex;align-items:center;gap:10px;padding:.75rem 1.6rem;border:1.5px solid #d1d5db;border-radius:10px;background:#fff;color:#1a1a1a;font-size:1rem;font-weight:500;cursor:pointer;box-shadow:0 1px 6px rgba(0,0,0,.09)">
  ...
  <p id="err" style="color:#dc2626;font-size:.82rem;margin-top:.75rem;min-height:1.2rem"></p>
</div>
<script>
const fc={cfg_json};
if(!firebase.apps.length) firebase.initializeApp(fc);
function signIn(){{
  document.getElementById('gb').textContent='Memuat...';
  const pv=new firebase.auth.GoogleAuthProvider();
  firebase.auth().signInWithPopup(pv)
    .then(r=>r.user.getIdToken())
    .then(tk=>{{
      const u=firebase.auth().currentUser;
      const url=new URL(window.parent.location.href);
      url.searchParams.set('fb_token',tk);
      url.searchParams.set('fb_uid',u.uid);
      url.searchParams.set('fb_email',u.email||'');
      url.searchParams.set('fb_name',u.displayName||'');
      url.searchParams.set('fb_photo',u.photoURL||'');
      window.parent.location.href=url.toString();
    }})
    .catch(e=>{{
      document.getElementById('err').textContent=e.message;
      document.getElementById('gb').textContent='Masuk dengan Google';
    }});
}}
</script>""".replace("{cfg_json}", cfg_json), height=320)
